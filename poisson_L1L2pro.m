function out = poisson_L1L2pro(U0,Img,orig, opts)
...
%     e = e + W(u) - d;    
    ...
    relError_1 =norm(u-orig,'fro')/norm(orig,'fro');
    if relError(k) < tol
        break;
    end
   mu=min(tau*mu,1e15);
   ...
   err(iter) =  relError(k);
   err_1(iter)= relError_1;
%     err(2)=1.0;
    snrval(iter) = snr(u, orig); 
...
out.sol                 = u;
out.snr_val                 =snrval;
out.relativeError       = err;
out.relativeError_1       = err_1;
out.times               =times;
out.OverallItration     = size(out.relativeError,2);  
end
%No of itr to converge
end
